...
describe('updating blog info', ()=> {
    test.only('updating likes succeeds if id is valid', async () => {
    ...
    test.only('fails with statuscode 400 if new value is invalid', async () => {
        const newLikes = "many"
    ...
    test.only('fails with statuscode 404 if blog is deleted', async () => {
        const validNonexistingId = await helper.nonExistingId()
